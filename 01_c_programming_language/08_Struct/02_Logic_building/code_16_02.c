 ...
 * @breif: A library management system using structs.
 * @description: 
 * This program simulates a library management sytem where:
...
void displayBorrowerDetails(struct Borrower *b,int count)
{
	...
	for(le = 0; le < count; le++)
	{
         ...
         printf("Author Nationality: %s\n",b[le].borrowedBook.bookAuthor.autNationality); 
	}
}
